...
Abstract:
    ...
    These are defined in internal.h.  If you choose to use a different
    primary include file, you should ensure they are defined there as well.
...
/*++
  ...
  Routine Description:
    ...
    This routine is called by COM in order to instantiate the NMEA Translator 
    UMDF driver callback object and do an initial query interface on it.
...
--*/
{
    ...
    *Interface = NULL;
    ...
    if (IsEqualCLSID(ClassId, GUID_DEVCLASS_NMEA) == false)
    {
        Trace(
            TRACE_LEVEL_ERROR,
