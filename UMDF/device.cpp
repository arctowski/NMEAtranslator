...
Abstract:
    ...
    This module contains the implementation of the NMEA Translator UMDF driver's
...
/*++
  ...
  Routine Description:
    ...
    This method creates and initializes an instance of the NMEA Translator UMDF driver's
    device callback object.
...
--*/
{
    ...
    //
    hr = m_FxDevice->CreateDeviceInterface(&GUID_DEVINTERFACE_NMEA,
        NULL);
    ...
    if (FAILED(hr))
    {
        Trace(
            ...
            &GUID_DEVINTERFACE_NMEA
        );

        return hr;
    }

    hr = m_FxDevice->AssignDeviceInterfaceState(&GUID_DEVINTERFACE_NMEA,
        NULL,
        TRUE);
...
/*++
  ...
  Routine Description:
    ...
    Since the NMEA Translator UMDF driver doesn't support any of the device events, this
    method simply calls the base class's BaseQueryInterface.

    If NMEA Translator is extended to include device event interfaces then this 
